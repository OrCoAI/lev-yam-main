...
> | **B2** | ~~`ANTHROPIC_API_KEY`~~ → **`CLAUDE_CODE_OAUTH_TOKEN`** (owner's subscription token, [ADR 0042](../decisions/0042-agent-workflows-run-on-the-subscription-token.md)) | Step 6's acceptance | The API-key route failed at $0 with no model call; owner confirmed 2026-09-21 the API account has no credits and chose not to fund it yet. **Owner: `claude setup-token` on the Mac, then `gh secret set CLAUDE_CODE_OAUTH_TOKEN`.** Then one `workflow_dispatch` of `quarterly-prep.yml` is the test |
